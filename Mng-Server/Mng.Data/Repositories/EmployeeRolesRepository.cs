...
namespace Mng.Data.Repositories
{
    public class EmployeeRolesRepository : IEmployeeRolesRepository
    {
        ...
        public async Task<EmployeeRoles> AddAsync(EmployeeRoles empRole)
        {
            ...
            //בדיקה  אם התאריך חייב להיות מאוחר או שווה לתאריך תחילת העבודה
            var existingDate = await _context.EmployeeRoles
               .FirstOrDefaultAsync(er => er.EmployeeId == empRole.EmployeeId && er.StartDate >= empRole.StartDate);
            if (existingDate != null)
            {
                throw new ArgumentException("Employee already has a role with a start date on or after the provided start date");
            }
