...
namespace Mng.Data.Repositories
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        {
            return await _context.Employees.Where(e => e.ActivityStatus).Include(e => e.Roles).ThenInclude(er => er.Role).ToListAsync();
        }
        public async Task<Employee> GetByIdAsync(int employeeId)
        {
            return await _context.Employees.Where(e => e.ActivityStatus).Include(e=>e.Roles).ThenInclude(j=>j.Role).FirstAsync(c => c.Id == employeeId);
        }
